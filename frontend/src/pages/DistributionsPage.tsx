...
function distRegionLabel(b: any, dash: string) {
  const r = (b?.region?.nameAr ?? '').trim();
  if (r) return r;
  const a = (b?.area ?? '').trim();
  if (a) return a;
  return dash;
...
export function DistributionsPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['distributions', status],
    queryFn: async () => (await api.get('/distributions', { params: { status: status || undefined } })).data,
  });
    ...
    <div className="space-y-4">
      ...
      <Card className="max-w-full overflow-x-auto p-0">
        {isLoading ? (
          ...
          <div className="p-10 text-center text-sm text-muted-foreground">{t('common.noRecords')}</div>
        ) : (
          <>
              ...
              <table className="w-full min-w-[1180px] text-sm">
                <thead className="bg-muted/40 text-start">
                  <tr className="border-b border-border">
                    ...
                    <th className="p-3">{t('distributions.colRegion')}</th>
                ...
                <tbody>
                  {rows.map((d: any) => (
                    <tr key={d.id} className="border-b border-border align-top hover:bg-muted/20">
                      ...
                      <td className="p-3">{distRegionLabel(d.beneficiary, t('common.dash'))}</td>
                ...
                <div key={d.id} className="space-y-3 rounded-lg border border-border bg-card p-4 text-sm">
                  ...
                  <dl className="grid gap-2 text-sm">
                    <div className="flex flex-col gap-0.5">
                      <dt className="text-muted-foreground">{t('distributions.colRegion')}</dt>
                      <dd className="font-medium">{distRegionLabel(d.beneficiary, t('common.dash'))}</dd>
                    </div>
                    <div className="flex flex-col gap-0.5">
