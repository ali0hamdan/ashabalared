...
import { deleteBlocked, forceDeleteForbidden } from '../common/http/delete-blocked';
...
export class DistributionService {
  ...
  private normalizeDistributionStatus(raw?: string): DistributionStatus | undefined {
  ...
  async list(actor: { userId: string; roleCode: RoleCode }, query: { status?: string; q?: string }) {
  ...
  async get(actor: { userId: string; roleCode: RoleCode }, id: string) {
    ...
    const d = await this.prisma.distributionRecord.findUnique({ where: { id }, include: distInclude });
  ...
  async create(actorId: string, dto: CreateDistributionDto) {
    ...
    for (const line of dto.items) {
      ...
      if (seenStock.has(line.stockItemId)) throw new BadRequestException('صنف مخزون مكرر في الطلب');

      seenStock.add(line.stockItemId);

      if (line.quantity < 1) throw new BadRequestException('الكمية يجب أن تكون أكبر من صفر');
      ...
      const stock = await this.prisma.stockItem.findUnique({
        ...
        select: {
          ...
          aidCategoryItem: { select: { id: true, name: true, aidCategoryId: true } },

        },

      });

      if (!stock?.aidCategoryItem) throw new BadRequestException('صنف مخزون غير صالح');

      if (line.quantity > stock.quantityOnHand) {

        throw new BadRequestException(`مخزون غير كافٍ لـ ${stock.aidCategoryItem.name}`);
  ...
  async assignDriver(actorId: string, id: string, driverId: string) {
    ...
    const dist = await this.prisma.distributionRecord.findUnique({ where: { id } });

    if (!dist) throw new NotFoundException();

    if (dist.status === DistributionStatus.ASSIGNED && dist.driverId === driverId) {

      const unchanged = await this.prisma.distributionRecord.findUnique({ where: { id }, include: distInclude });
    ...
    if (dist.status !== DistributionStatus.PENDING) {
      ...
      throw new BadRequestException('يمكن تعيين السائق للتوزيعات في حالة الانتظار فقط');
  ...
  async confirmDelivery(actorUserId: string, id: string, dto: DeliverDistributionDto) {
    ...
    if (dist.status !== DistributionStatus.ASSIGNED) {
      ...
      throw new BadRequestException('يمكن تأكيد التسليم للتوزيعات المعيّنة للسائق فقط');

    }

    if (!dist.items.length) throw new BadRequestException('التوزيع لا يحتوي بنوداً');
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      for (const line of dist.items) {
        ...
        const qtyDelivered = deliveredByLine.get(line.id) ?? line.quantityPlanned;
        ...
        if (qtyDelivered > stock.quantityOnHand) {
          ...
          throw new BadRequestException('الكمية المسلّمة تتجاوز المتاح في المخزون');
  ...
  async cancel(actorId: string, id: string) {
    ...
    if (dist.status !== DistributionStatus.PENDING && dist.status !== DistributionStatus.ASSIGNED) {

      throw new BadRequestException('يمكن إلغاء التوزيعات في حالة الانتظار أو المعيّنة فقط');
  ...
  async remove(actorId: string, id: string) {
    ...
    if (dist.status === DistributionStatus.DELIVERED) {
      ...
      throw deleteBlocked('Delivered distributions cannot be deleted; historical integrity is preserved.', ['invalidStatus'], {
  ...
  async forceRemove(actor: AuthUser, id: string, confirmationText: string, reason?: string) {

    if (actor.roleCode !== RoleCode.SUPER_ADMIN && actor.roleCode !== RoleCode.ADMIN) throw new ForbiddenException();
    ...
    if (dist.status === DistributionStatus.DELIVERED) {
      ...
      throw forceDeleteForbidden('Delivered distributions cannot be force-deleted; historical integrity is preserved.');
